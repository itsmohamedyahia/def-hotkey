...
def add_to_startup():
    ...
    if is_frozen:
        ...
        python_exe = sys.executable.replace("python.exe", "pythonw.exe") # Use pythonw for no console
...
def add_to_startup_windows(command):
    ...
    try:
        ...
        with reg_key:
            winreg.SetValueEx(reg_key, "WordLookupApp", 0, winreg.REG_SZ, command)
        print("Successfully added Word Lookup App to Windows startup.")
...
Terminal=false
"""
        with open(os.path.join(autostart_dir, "word_lookup.desktop"), "w") as f:
            f.write(desktop_entry.strip())
        print("Successfully added Word Lookup App to Linux startup.")
    except Exception as e:
...
def add_to_startup_macos(command):
    ...
    try:
        ...
        plist_name = "com.user.wordlookupapp.plist"
...
"""
        ...
        print("Successfully added Word Lookup App to macOS startup.")
...
def get_config_path():
    ...
    app_name = "WordLookupApp"
    if sys.platform == "win32":
        # C:\Users\<User>\AppData\Roaming\WordLookupApp
        config_dir = os.path.join(os.environ['APPDATA'], app_name)
    else:
        # ~/.config/WordLookupApp
...
def setup_auto_start():
    ...
    if not os.path.exists(config_file):
        ...
        print("First run detected. Adding application to startup...")
        try:
            add_to_startup()
...
class WordLookupApp:
    def __init__(self, root):
        ...
        self.tray_icon = None
        # Remove the old flags, we'll use different ones
    ...
    def run_lookup_and_update_gui(self, word):
        ...
        # --- MODIFICATION HERE ---
        # We now pass the original `word` to the display function so it knows what to highlight.
    ...
    def display_results(self, content, title="Results", search_word=None):
        ...
        # keep it above briefly so it grabs attention
        ...
        # --- Highlighting logic (fixed import: tkinter.font as tkfont) ---
        if search_word:
            try:
                ...
                # fallback: use a simple font tuple if font object fails
                try:
                    default_font_actual = tkfont.nametofont("TkDefaultFont").actual()
                    bold_font_tuple = (default_font_actual['family'], default_font_actual['size'], 'bold')
                    self.result_text_widget.tag_configure("highlight", font=bold_font_tuple)
                except Exception:
                    pass

            pos = '1.0'
            pattern = fr'\y{re.escape(search_word)}\y'  # Tcl/Tk regexp word boundaries
            while True:
        ...
        # Force layout so window has actual requested pixel size, then center it
        self.result_window.update_idletasks()

        # Use requested size to compute exact placement (more reliable)
        win_w = self.result_window.winfo_reqwidth()
        win_h = self.result_window.winfo_reqheight()
        screen_w = self.result_window.winfo_screenwidth()
        screen_h = self.result_window.winfo_screenheight()
        x = (screen_w // 2) - (win_w // 2)
        y = (screen_h // 2) - (win_h // 2)

        # Set both size and position to avoid surprises
        ...
        try:
            ...
            pass
        # remove topmost after it has been shown so it doesn't stay above all apps
    ...
    def update_result_window(self, content, word):
        ...
        if self.result_window and self.result_window.winfo_exists() and self.result_text_widget:
            try:
                ...
                # Force layout then compute requested size and re-center
                self.result_window.update_idletasks()
                win_w = self.result_window.winfo_reqwidth()
                win_h = self.result_window.winfo_reqheight()
                screen_w = self.result_window.winfo_screenwidth()
            ...
            try:
                ...
                if os.path.exists(image_path):
                    if icon_name.endswith('.ico'):
                        # For ICO files, you might need to handle them differently
                        img = Image.open(image_path)
                        # Convert ICO to PNG format if needed
                        if img.format == 'ICO':
...
# Alternative approach using a simpler hotkey setup
...
def main():
    global app
    print("Starting Word Lookup App...")
